...
object ClientWriter {
  ...
  def writeObject(
    ...
    val fields = typedef.fields.map(
      collectFieldInfo(_, objectName, typesMap, mappingClashedTypeNames)
    )
    val view =
      if (genView && typedef.fields.length <= MaxTupleLength)
        "\n  " + writeView(
          typedef.name,
          fields.map(_.typeInfo),
  ...
  def writeView(
    ...
    val viewFunctionArguments: List[String] =
      fields.collect {
        case field if field.arguments.nonEmpty =>
          writeArgumentFields(
            field.arguments
              .map(a => a.copy(name = argumentName(field.name, a.name))),
            mappingClashedTypeNames
          )
      }
    ...
    val viewClassFields: List[String] =
      fields.map {
        case field @ FieldTypeInfo(_, _, outputType, _, _, _, Some(_)) =>
          ...
          val tpe =
            if (outputType.contains("[A]"))
              outputType.replace("[A]", s"[$tpeName]")
        ...
        .map {
          case field @ FieldTypeInfo(
            ...
            val argsPart = withRoundBrackets(
              field.arguments.map(a => argumentName(field.name, a.name))
            )
    ...
    val viewFunction: String =
      fields match {
        ...
        case other =>
          val unapply =
            fields.tail.foldLeft(safeUnapplyName(fields.head.rawName)) {
              case (acc, field) =>
    ...
    val typeParams =
      if (genericSelectionFieldTypes.nonEmpty)
        genericSelectionFieldTypes.map(_._2).mkString("[", ", ", "]")
      else ""

    val viewFunctionArgs = withRoundBrackets(viewFunctionArguments)
    val viewFunctionSelectionArgs = withRoundBrackets(
      viewFunctionSelectionArguments
    )
    ...
    t match {
      case NamedType(name, true) =>
        if (name == typeName) s"encode($fieldName)"
          ...
          s"implicitly[ArgEncoder[${mapTypeName(name, mappingClashedTypeNames)}]].encode($fieldName)"
  ...
  def getTypeLetter(
    ...
    if (!typesMap.contains(letter)) letter
    else getTypeLetter(typesMap, letter + "A")
  ...
  def writeField(
    ...
    writeFieldInfo(
      collectFieldInfo(field, typeName, typesMap, mappingClashedTypeNames)
    )
  ...
  def collectFieldInfo(
    ...
    val fieldType =
      safeTypeName(getTypeName(field.ofType), mappingClashedTypeNames)
    ...
    val argBuilder = field.args match {
      case Nil => ""
      case list =>
        s", arguments = List(${list
          .map(arg => s"""Argument("${arg.name}", ${safeName(arg.name)})""")
          .mkString(", ")})"
    }
  ...
  def writeType(t: Type, mappingClashedTypeNames: Map[String, String]): String =
    t match {
      ...
      case NamedType(name, false) =>
        s"Option[${mapTypeName(name, mappingClashedTypeNames)}]"
      case ListType(ofType, true) =>
        s"List[${writeType(ofType, mappingClashedTypeNames)}]"
      case ListType(ofType, false) =>
        s"Option[List[${writeType(ofType, mappingClashedTypeNames)}]]"
    }
  ...
  def writeTypeBuilder(t: Type, inner: String): String =
    t match {
      ...
      case ListType(of, false) =>
        s"OptionOf(ListOf(${writeTypeBuilder(of, inner)}))"
    }
